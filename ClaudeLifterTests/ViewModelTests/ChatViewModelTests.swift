...
struct ChatViewModelTests {
    ...
    func makeViewModel(
        ...
        // Return container to keep it alive for the duration of the test
    ...
    func sendMessageYieldsAssistantResponse() async throws {
        let (vm, _, _) = try makeViewModel(events: [.text("Do 3 sets of 8."), .complete])
    ...
    func sendMessageClearsStreamingText() async throws {
        let (vm, _, _) = try makeViewModel(events: [.text("Hello!"), .complete])
    ...
    func clearChatResetsState() async throws {
        let (vm, _, _) = try makeViewModel(events: [.text("Hi"), .complete])
        await vm.sendMessage("Hello")
    ...
    func toolUseTriggersExecution() async throws {
        let (vm, mock, _) = try makeViewModel(events: [
    ...
    func textChunksAccumulated() async throws {
        let (vm, _, _) = try makeViewModel(events: [.text("Hello"), .text(" world"), .complete])
    ...
    func systemPromptIncludesPreferences() async throws {
        ...
        _ = container // keep container alive until after sendMessage
    }
}
